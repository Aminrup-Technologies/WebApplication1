...
namespace WebApplication1.bussiness.production
{
    public partial class usertoggle : System.Web.UI.Page
    {
        ...
        protected void btnSubmitAadhaar_Click_OLD(object sender, EventArgs e)
        {
            ...
            // Aadhaar image upload validation
            ...
            {
                string folderPath = Server.MapPath("~/Uploads/Aadhar/");
                if (!Directory.Exists(folderPath))
                    Directory.CreateDirectory(folderPath);

                fileName = "Aadharcard_" + WorkmanSL + Path.GetExtension(fuAadhaarImage.FileName);
                string savePath = Path.Combine(folderPath, fileName);
                fuAadhaarImage.SaveAs(savePath);
                relativePath = "~/Uploads/Aadhar/" + fileName;
            }
                    ...
                    using (SqlTransaction tx = conn.BeginTransaction())
                    {
                        try
                        {
                            ...
                            using (SqlCommand checkCmd = new SqlCommand(checkQuery, conn, tx))
                            {
                                ...
                                using (SqlCommand cmd = conn.CreateCommand())
                                {
                                    ...
                                    if (recordExists > 0)
                                    {
                                        ...
                                        if (fuAadhaarImage.HasFile)
                                            cmd.Parameters.AddWithValue("@AadhaarImagePath", relativePath);
                            ...
                            UpdateAadhaarDocs(conn, tx,
                                workmanSL: WorkmanSL,
                                aadhaarYesNo: 1, aadhaarID: aadhaar, aadhaarPath: relativePath, aadhaarDate: issueDate,
                                updatedByWrk: WorkmanSL, updatedByName: Session["FullName"]?.ToString()
                            );
        ...
        protected void btnSubmitPAN_Click_OLD(object sender, EventArgs e)
        {
            lblPanMessage.Text = "";
            ...
            {
                string fileName = "PANCard_" + WorkmanSL + Path.GetExtension(fuPANImage.FileName);
                string serverPath = Server.MapPath("~/Uploads/PAN/");
                if (!Directory.Exists(serverPath))
                    Directory.CreateDirectory(serverPath);
                fuPANImage.SaveAs(serverPath + fileName);
                imagePath = "~/Uploads/PAN/" + fileName;
            }
            ...
            //               SET PANNumber=@PANNumber, PANName=@PANName, IssueDate=@IssueDate, 
            //                   PANImagePath=ISNULL(NULLIF(@ImagePath, ''), PANImagePath)
                    ...
                    using (SqlTransaction tx = con.BeginTransaction())
                    {
                        try
                        {
                            ...
                            if (ViewState["UserPANID"] != null)
                            {
                                // UPDATE existing
                                int panID = Convert.ToInt32(ViewState["UserPANID"]);
                                string updateQuery = @"UPDATE UserPANCard SET PANNumber=@PANNumber, PANName=@PANName, IssueDate=@IssueDate, 
                                       PANImagePath=ISNULL(NULLIF(@ImagePath, ''), PANImagePath)
                                   WHERE UserPANID=@UserPANID";
                            ...
                            UpdatePanDocs(con, tx,
                                workmanSL: WorkmanSL,
                                panYesNo: 1, panNo: panNumber, panPath: imagePath, panDate: issueDate,
                                updatedByWrk: WorkmanSL, updatedByName: Session["FullName"]?.ToString()
                            );
        ...
        protected void btnSubmitBank_Click_OLD(object sender, EventArgs e)
        {
            string WorkmanSL = Session["WORKMAN"]?.ToString();
            if (string.IsNullOrEmpty(WorkmanSL))
            {
            ...
            {
                string folderPath = Server.MapPath("~/Uploads/Bank/");
                if (!Directory.Exists(folderPath))
                    Directory.CreateDirectory(folderPath);

                fileName = "Bank_" + WorkmanSL + Path.GetExtension(fuBankImage.FileName);
                string savePath = Path.Combine(folderPath, fileName);
                fuBankImage.SaveAs(savePath);
                relativePath = "~/Uploads/Bank/" + fileName;
            }
                    ...
                    using (SqlTransaction tx = conn.BeginTransaction())
                    {
                        try
                        {
                            ...
                            SET AccountNumber = @AccountNumber,
                                ...
                                if (fuBankImage.HasFile)
                                    cmd.Parameters.AddWithValue("@BankImagePath", relativePath);

                                lblBankMessage.Text = "Bank details updated successfully!";
                            }
                            else
                            {
                                // Insert logic
                            ...
                            VALUES (@AccountNumber, @IFSCCode, @BankName, @BankImagePath, GETDATE(), @AccountHolderName, @UserID)";
                                ...
                                lblBankMessage.Text = "Bank details saved successfully!";
                            }
                            ...
                            UpdateBankDocs(conn, tx,
                                workmanSL: WorkmanSL,
                                bankYesNo: 1, bankPath: relativePath, bankDate: DateTime.Now,
                                updatedByWrk: WorkmanSL, updatedByName: Session["FullName"]?.ToString()
                            );
        ...
        protected void btnSubmitEducation_Click_OLD(object sender, EventArgs e)
        {
            ClearAllHighlights();
            ...
            int yearUG = chkUGNA.Checked ? 0 : TryParseYear(txtUGYear.Text.Trim());
            int yearPG = chkPGNA.Checked ? 0 : TryParseYear(txtPGYear.Text.Trim());

            // Validate 10th
            if (!ValidateBasicFields(txt10Board, txt10Year, txt10Marks))
            {
                hasError = true;
            }
            else if (!IsValid10thYear(year10, currentYear))
            {
            ...
            // Validate 12th
            if (!ValidateBasicFields(txt12Board, txt12Year, txt12Marks))
            {
                hasError = true;
            }
            else if (!IsValid12thYear(year10, year12, currentYear))
            {
            ...
            if (!chkUGNA.Checked)
            {
                if (!ValidateBasicFields(txtUGBoard, txtUGYear, txtUGMarks))
                {
                    hasError = true;
                }
                else if (!IsValidUGYear(year12, yearUG, currentYear))
                {
            ...
            if (!chkPGNA.Checked)
            {
                if (!ValidateBasicFields(txtPGBoard, txtPGYear, txtPGMarks))
                {
                    hasError = true;
                }
                else if (!IsValidPGYear(yearUG, yearPG, currentYear))
                {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["DbConn"].ConnectionString))
            {
                ...
                using (SqlTransaction tx = con.BeginTransaction())
                {
                    ...
                    {
                        // 10th & 12th (no NA)
                        string file10 = SaveOrUpdateEducation_OLD(con, tx, userID, "10th", "SSC", txt10Board.Text.Trim(), txt10Year.Text.Trim(), txt10Marks.Text.Trim(), fu10Image, false);
                        string file12 = SaveOrUpdateEducation_OLD(con, tx, userID, "12th", "HSC", txt12Board.Text.Trim(), txt12Year.Text.Trim(), txt12Marks.Text.Trim(), fu12Image, false);

                        // UG
                        bool isUGNA = chkUGNA.Checked;
                        string fileUG = SaveOrUpdateEducation_OLD(
                            con,
                            tx,
                            userID,
                            "UG",
                        ...
                        // PG
                        bool isPGNA = chkPGNA.Checked;
                        string filePG = SaveOrUpdateEducation_OLD(
                            con,
                            tx,
                            userID,
                            "PG",
                        ...
                        int tenFlag = (file10 != null && file10 != "NA") ? 1 : 0;
                        int twelveFlag = (file12 != null && file12 != "NA") ? 1 : 0;
                        int gradFlag = (fileUG != null && fileUG != "NA") ? 1 : 0;

                        // Update docs table (only education-related columns)
                        UpdateEducationDocs(con, tx,
                            workmanSL: userID,
                            tenYesNo: tenFlag, tenPath: (tenFlag == 1 ? file10 : null), tenDate: tenFlag == 1 ? (DateTime?)DateTime.Now : null,
                            twelveYesNo: twelveFlag, twelvePath: (twelveFlag == 1 ? file12 : null), twelveDate: twelveFlag == 1 ? (DateTime?)DateTime.Now : null,
                            gradYesNo: gradFlag, gradPath: (gradFlag == 1 ? fileUG : null), gradDate: gradFlag == 1 ? (DateTime?)DateTime.Now : null,
        ...
        protected void btnSubmitEducation_Click_10Dec25(object sender, EventArgs e)
        {
            ...
            if (!ValidateBasicFields(txt10Board, txt10Year, txt10Marks) || !IsValid10thYear(year10, currentYear)) { AddErrorHighlight(txt10Year); hasError = true; }
            if (!ValidateBasicFields(txt12Board, txt12Year, txt12Marks) || !IsValid12thYear(year10, year12, currentYear)) { AddErrorHighlight(txt12Year); hasError = true; }
            if (!chkUGNA.Checked)
            {
                if (!ValidateBasicFields(txtUGBoard, txtUGYear, txtUGMarks) || !IsValidUGYear(year12, yearUG, currentYear)) { AddErrorHighlight(txtUGYear); hasError = true; }
            }
            if (!chkPGNA.Checked)
            {
                if (!ValidateBasicFields(txtPGBoard, txtPGYear, txtPGMarks) || !IsValidPGYear(yearUG, yearPG, currentYear)) { AddErrorHighlight(txtPGYear); hasError = true; }
            }
            ...
            // === Save uploaded files FIRST (outside transaction) ===
            // call SaveEmployeeDocument(fileUpload, workmanSL, docType)
            string file10 = "NA", file12 = "NA", fileUG = "NA", filePG = "NA";
            try
            {
                if (fu10Image.HasFile) file10 = SaveEmployeeDocument(fu10Image, userID, "Education/10th");
                if (fu12Image.HasFile) file12 = SaveEmployeeDocument(fu12Image, userID, "Education/12th");
                if (fuUGImage.HasFile) fileUG = SaveEmployeeDocument(fuUGImage, userID, "Education/UG");
                if (fuPGImage.HasFile) filePG = SaveEmployeeDocument(fuPGImage, userID, "Education/PG");
            }
            catch (Exception exFile)
            {
                lblEducationStatus.Text = "Error saving file(s): " + exFile.Message;
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["DbConn"].ConnectionString))
            {
                ...
                using (SqlTransaction tx = con.BeginTransaction())
                {
                    try
                    {
                        // Use SaveOrUpdateEducation that accepts documentPath (string) and uses con, tx internally
                        string saved10 = SaveOrUpdateEducation(con, tx, userID, "10th", "SSC", txt10Board.Text.Trim(), txt10Year.Text.Trim(), txt10Marks.Text.Trim(), file10, false);
                        string saved12 = SaveOrUpdateEducation(con, tx, userID, "12th", "HSC", txt12Board.Text.Trim(), txt12Year.Text.Trim(), txt12Marks.Text.Trim(), file12, false);

                        bool isUGNA = chkUGNA.Checked;
                        string savedUG = SaveOrUpdateEducation(con, tx, userID, "UG", "B.Tech", isUGNA ? "N/A" : txtUGBoard.Text.Trim(), isUGNA ? "0" : txtUGYear.Text.Trim(), isUGNA ? "N/A" : txtUGMarks.Text.Trim(), fileUG, isUGNA);

                        bool isPGNA = chkPGNA.Checked;
                        string savedPG = SaveOrUpdateEducation(con, tx, userID, "PG", "M.Tech", isPGNA ? "N/A" : txtPGBoard.Text.Trim(), isPGNA ? "0" : txtPGYear.Text.Trim(), isPGNA ? "N/A" : txtPGMarks.Text.Trim(), filePG, isPGNA);
        ...
        // Returns stored DocumentPath (or "NA" if none).
        private string SaveOrUpdateEducation_OLD(SqlConnection con, SqlTransaction tx,
            string userID,
            string qualificationType,   // e.g. "10th", "12th", "UG", "PG"
            string qualification,       // e.g. "SSC", "HSC", "B.Tech"
            string boardOrInstitute,
            string passingYear,
            string marksOrGrade,
            ...
            bool isNA)
        {
            // Save uploaded file first (if any) and get relative path. If none, "NA".
            string documentPath = "NA";
            if (fileUpload != null && fileUpload.HasFile)
            {
                // Ensure GetUploadedFileName is file-system oriented or safe to call inside tx.
                //documentPath = GetUploadedFileName(fileUpload, qualificationType);
                documentPath = SaveEmployeeDocument(fileUpload, workman, "Education");
                if (string.IsNullOrWhiteSpace(documentPath)) documentPath = "NA";
            }

            // Use UserID + QualificationType as uniqueness key
            string checkSql = "SELECT COUNT(*) FROM EducationDetails WHERE UserID = @UserID AND QualificationType = @QualificationType";
            using (SqlCommand cmdCheck = new SqlCommand(checkSql, con, tx))
            {
                cmdCheck.Parameters.AddWithValue("@UserID", userID);
                ...
                WHERE UserID = @UserID AND QualificationType = @QualificationType";
                    using (SqlCommand cmdUpd = new SqlCommand(updateSql, con, tx))
                    {
                        ...
                        cmdUpd.Parameters.AddWithValue("@PassingYear", (object)passingYear ?? DBNull.Value);
                        cmdUpd.Parameters.AddWithValue("@MarksOrGrade", (object)marksOrGrade ?? DBNull.Value);
                        cmdUpd.Parameters.AddWithValue("@DocumentPath", documentPath == "NA" ? "" : (object)documentPath);
                        cmdUpd.Parameters.AddWithValue("@IsNA", isNA ? 1 : 0);
                        cmdUpd.Parameters.AddWithValue("@UserID", userID);
                        cmdUpd.Parameters.AddWithValue("@QualificationType", qualificationType);
                        cmdUpd.ExecuteNonQuery();
                ...
                {
                    // Insert new row (CreatedDate has default GETDATE())
                    string insertSql = @"
                INSERT INTO EducationDetails
                (QualificationType, Qualification, BoardOrInstitute, MarksOrGrade, PassingYear, DocumentPath, UserID, IsNA)
                VALUES
                (@QualificationType, @Qualification, @BoardOrInstitute, @MarksOrGrade, @PassingYear, @DocumentPath, @UserID, @IsNA)";
                    using (SqlCommand cmdIns = new SqlCommand(insertSql, con, tx))
                    {
                        ...
                        cmdIns.Parameters.AddWithValue("@MarksOrGrade", (object)marksOrGrade ?? DBNull.Value);
                        cmdIns.Parameters.AddWithValue("@PassingYear", (object)passingYear ?? DBNull.Value);
                        cmdIns.Parameters.AddWithValue("@DocumentPath", documentPath == "NA" ? "" : (object)documentPath);
                        cmdIns.Parameters.AddWithValue("@UserID", userID);
                        cmdIns.Parameters.AddWithValue("@IsNA", isNA ? 1 : 0);
                        cmdIns.ExecuteNonQuery();
                    }
