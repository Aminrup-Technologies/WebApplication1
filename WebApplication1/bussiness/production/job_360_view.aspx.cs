...
namespace WebApplication1.bussiness.production
{
    public partial class job_360_view : System.Web.UI.Page
    {
            ...
            DateTime? dtApproval)
        {
            ...
            if (string.Equals(GetSafeString(row, "EntryExit", ""), JobStatusConstants.EntryExitExit, StringComparison.OrdinalIgnoreCase))
            {
        ...
        private void ApplyAdminOverrideVisibility(string approvalStatus, string masterCode, int actualWorkerCount)
        {
            ...
            if (masterCode == JobStatusConstants.CodeCreated)
            {
                btn_Act_ForcePermitBypass.Visible = true;
                btn_Act_CancelShift.Visible = true;
            }

            if (masterCode == JobStatusConstants.CodeInPunch && actualWorkerCount == 0)
            {
                btn_Act_ResetToCreated.Visible = true;
                btn_Act_CancelShift.Visible = true;
            }
        ...
        {
            if ((isBlocked || jobStatus == "Blocked") && entryExitStatus != JobStatusConstants.EntryExitExit && IsAdmin())
            {
        ...
        {
            bool isCreated = entryExitStatus == JobStatusConstants.EntryExitCreated;
            bool isEntry = entryExitStatus == JobStatusConstants.EntryExitEntry;
            bool isExit = entryExitStatus == JobStatusConstants.EntryExitExit;
        ...
        {
            if (entryExitStatus == JobStatusConstants.EntryExitEntry && dtFirstInPunchLogic.HasValue && dtCreatedDate.HasValue && dtCreatedDate.Value.Date < DateTime.Now.Date)
            {
                btn_Act_ForceOut.Visible = true;
            }
        }
        ...
        {
            if ((entryExitStatus == JobStatusConstants.EntryExitEntry || entryExitStatus == JobStatusConstants.EntryExitCreated) && dtFirstInPunchLogic.HasValue)
            {
                if (IsAdmin()) btn_Act_ForceOut.Visible = true;
                lbl_bottleneck.Text += " Admin must Force OUT-Punch to close this shift.";
            }
