...
{
    public partial class gen_ninl_f17 : System.Web.UI.Page
    {
        ...
        protected void GridView_DataBound(object sender, EventArgs e)
        {
            for (int i = 0; i <= GridView.Rows.Count - 1; i++)
            {
                ...
                decimal adv = AdvanceAmt + FinesAmt + OthersAmt;
                ...
                Label lbl_grossded = (Label)GridView.Rows[i].FindControl("lbl_grossded");
                Label lbl_netpayfnl = (Label)GridView.Rows[i].FindControl("lbl_netpayfnl");

                if (FixRateSalary > 12000)
                {
                    PayRoll.EmployeeOthersPayCalculations1(TotalPresents, CalWorkingDays, DaVdaAmount, ref DaVdaPay);
                    PayRoll.EmployeeOthersPayCalculations2_NINL(TotalPresents, CalWorkingDays, HRAAmount, FixRateSalary, ref HRAPay);
                    PayRoll.EmployeeOthersPayCalculations3_NINL(TotalPresents, CalWorkingDays, ConvAmount, FixRateSalary, ref ConvPay);
                    PayRoll.EmployeeOthersPayCalculations4(TotalPresents, CalWorkingDays, MedAmount, ref MedPay);
                    PayRoll.EmployeeOthersPayCalculations5_NINL(TotalPresents, CalWorkingDays, WashAmount, BasicSalary, ref WashPay);
                ...
                decimal maxgross = 0.0m;

                otherpay = FixRateSalary - (BasicSalary + HRAPay + ConvPay + WashPay);
                grosstotal = BasicSalary + HRAPay + ConvPay + WashPay + otpay + otherpay;
                GrossAmount = grosstotal - (ConvPay + WashPay) - adv;  //minus the advance amount
                ...
                lbl_actualgross.Text = grosstotal.ToString();
                //----------------Gross Calculation & NET Payment 2 -------------------------------//

                if (GrossAmount > GorssBreaker)
                {
                    maxgross = GorssBreaker;                    
                }
                else
                {
                    maxgross = GrossAmount;
                }
                netpay1 = maxgross;
                //--------------------- ESIC pay---------------------------------------------------//

                decimal esicpay = 0.0m;
                esicpay = Math.Round(maxgross * 0.0075m, 0);
                lbl_esicpay.Text = esicpay.ToString();

                lbl_grossamount.Text = maxgross.ToString();
                lbl_otherspay.Text = otherpay.ToString();

                decimal ttldeductions = PFPay + esicpay;
                lbl_ttlded.Text = adv.ToString();

                decimal grossded = AdvanceAmt + FinesAmt + OthersAmt + ttldeductions;
                lbl_grossded.Text = grossded.ToString();

                //--------------------- NET Payment -------------------------------------------------//
                decimal p = maxgross - PFPay - esicpay;
                //netpay1 = p;
                netpay1final = p + ConvPay + WashPay;
                netpay2 = GrossAmount - maxgross;

                lbl_netpay1.Text = Math.Round(netpay1,0).ToString();
                lbl_netpayfnl.Text = Math.Round(netpay1final,0).ToString();
                lbl_netpay2.Text = Math.Round(netpay2,0).ToString();
            }
